...
class MapPlotter:
    ...
    def plot_map(self, user_point_gpd, highest_point_gpd, buffer_img, buffer_out_transform, shortest_path_gpd):
        ...
        def scale_bar(axe, length=None, location=(0.88, 0.05), linewidth=3):
            """
            ax is the axes to draw the scalebar on.
            length is the length of the scalebar in km.
            location is center of the scalebar in axis coordinates.
            (ie. 0.88 is the right of the plot)
            linewidth is the thickness of the scalebar.
            """
            # Get the limits of the axis in lat long
            llx0, llx1, lly0, lly1 = axe.get_extent(crs.PlateCarree())
            # Make tmc horizontally centred in the middle of the map,
            # vertically at scale bar location
            sbllx = (llx1 + llx0) / 2
            sblly = lly0 + (lly1 - lly0) * location[1]
            tmc = crs.TransverseMercator(sbllx, sblly)
            # Get the extent of the plotted area in coordinates in metres
            x0, x1, y0, y1 = axe.get_extent(tmc)
            # Turn the specified scalebar location into coordinates in metres
            sbx = x0 + (x1 - x0) * location[0]
            sby = y0 + (y1 - y0) * location[1]

            # Calculate a scale bar length if none has been given
            # (There os probably a more pythonic way of rounding the number but this works)
            if not length:
            ...
            # Plot the scalebar
            axe.plot(bar_xs, [sby, sby], transform=tmc, color='k', linewidth=linewidth)
            # Plot the scalebar label
            axe.text(sbx, sby, str(length) + ' km', transform=tmc, horizontalalignment='center',
